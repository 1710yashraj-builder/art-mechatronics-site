...
1. ~~**Phone number**~~ — **RESOLVED 2026-07-25.** Anurag confirmed three regional numbers: India `+91 80903 15151`, UAE `+971 50 216 7151`, Thailand `+66 838291089`; email `info@artmechatronics.com` confirmed correct. Because a `wa.me`/`tel:` link can only target one number, this shipped as a **region picker**: every WhatsApp/quote CTA opens an India/UAE/Thailand chooser and routes to that number, while the footer and Contact page list all three as tap-to-call. Single source of truth = `brand.contacts` in `js/data.js`; the picker lives in `js/layout.js` and the baked `wa.me/<India>` href remains the no-JS fallback. Old number `8191848660`: zero occurrences remaining. The § "Phone swap" procedure below is superseded by this — to change a number now, edit `brand.contacts` (and the `BRAND` block in `build/generate.js` for the fallback), then regenerate → validate → repackage → redeploy.
...
1. GoDaddy → DNS management for artmechatronics.com → **screenshot/export EVERY record** (A, CNAME, MX, TXT, NS, their values AND TTLs) and paste the picture/table into this file under "DNS zone as of &lt;date&gt;". This is the rollback baseline; without it nothing can be restored. Known today: apex A = `132.148.180.151` (⚠️ **CORRECTED 2026-07-25 — this is NOT an empty page.** It serves a **live WordPress site** with hundreds of Google-crawlable pages, and its https certificate is currently broken — issued for `05p.595.mytemp.website`. None of those legacy URLs exist on the new site, so cutting over without a redirect map deletes the client's existing search footprint. Before DNS: archive that WordPress content, capture its Analytics/Search Console baseline, and build the redirect map); MX = secureserver.net (⚠️ **the client's business email — info@/careers@ — lives on these MX records**).
...
2. **Netlify itself unreachable/broken** → GoDaddy DNS: restore the exact records recorded in STEP 0. Takes up to the record TTL to propagate (600s if the day-before step was done; up to 1 hour otherwise). Understand what this does: **CORRECTED 2026-07-25** — the old A record serves the client's **live WordPress site**, not an empty page, so a DNS rollback actually restores a working (if outdated) site. That makes rollback safer than this runbook originally claimed — but it is still a last resort, not plan A.
