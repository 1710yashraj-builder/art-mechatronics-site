...
(function () {
  ...
  const picker = (function () {
    ...
    function build() {
      ...
      style.id = "rpk-style";
      style.textContent =
        ".rpk-ov{position:fixed;inset:0;z-index:9999;display:none;align-items:center;justify-content:center;padding:1.2rem;background:rgba(11,44,94,.55);backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px)}" +
        ".rpk-ov.rpk-open{display:flex}" +
        ".rpk-panel{width:100%;max-width:430px;background:var(--white,#fff);border-radius:var(--r-lg,14px);box-shadow:var(--shadow-lg,0 24px 60px rgba(11,44,94,.18));overflow:hidden;animation:rpk-in .18s var(--ease,ease)}" +
        "@keyframes rpk-in{from{opacity:0;transform:translateY(10px) scale(.98)}to{opacity:1;transform:none}}" +
        ".rpk-head{display:flex;align-items:center;gap:.65rem;padding:1rem 1.15rem;border-bottom:1px solid var(--line,#dde3ec)}" +
        ".rpk-ic{width:36px;height:36px;flex:0 0 36px;border-radius:9px;display:grid;place-items:center;color:#fff}" +
        ".rpk-wa .rpk-ic{background:#25D366}.rpk-call .rpk-ic{background:#00A24C}" +
        ".rpk-head h3{margin:0;font:700 1.05rem/1.2 var(--font,sans-serif);color:var(--ink,#10233f)}" +
        ".rpk-head .rpk-sub{margin:.1rem 0 0;font-size:.8rem;color:var(--steel,#5c6678)}" +
        ".rpk-x{margin-left:auto;border:0;background:none;cursor:pointer;color:var(--steel,#5c6678);font-size:1.5rem;line-height:1;padding:.1rem .3rem;border-radius:6px}" +
        ".rpk-x:hover{background:var(--mist,#f4f7fb);color:var(--ink,#10233f)}" +
        ".rpk-list{padding:.7rem;display:flex;flex-direction:column;gap:.5rem}" +
        ".rpk-opt{display:flex;align-items:center;gap:.8rem;width:100%;text-align:left;padding:.62rem .9rem;border:1px solid var(--line,#dde3ec);border-radius:var(--r,9px);background:var(--white,#fff);cursor:pointer;transition:border-color .15s,background .15s,transform .08s;font-family:var(--font,sans-serif)}" +
        ".rpk-opt:hover,.rpk-opt:focus-visible{border-color:var(--blue,#1657b0);background:var(--blue-soft,#eaf2fd);outline:none}" +
        ".rpk-opt:active{transform:translateY(1px)}" +
        ".rpk-flag{font-size:1.5rem;flex:0 0 auto;line-height:1}" +
        ".rpk-r{font:700 1rem/1.15 var(--font,sans-serif);color:var(--ink,#10233f);display:block}" +
        ...
        ".rpk-num{margin-left:auto;font:600 .9rem/1 var(--font-mono,monospace);color:var(--blue,#1657b0);white-space:nowrap}";
      document.head.appendChild(style);

      overlay = document.createElement("div");
      overlay.className = "rpk-ov";
      overlay.innerHTML =
        '<div class="rpk-panel" role="dialog" aria-modal="true" aria-labelledby="rpk-title">' +
        '<div class="rpk-head"><span class="rpk-ic" aria-hidden="true"></span>' +
        '<div><h3 id="rpk-title"></h3><p class="rpk-sub">Choose your region</p></div>' +
        '<button class="rpk-x" type="button" aria-label="Close">×</button></div>' +
        '<div class="rpk-list"></div></div>';
      document.body.appendChild(overlay);

      overlay.querySelector(".rpk-x").addEventListener("click", close);
      overlay.addEventListener("click", e => { if (e.target === overlay) close(); });
      document.addEventListener("keydown", e => {
        if (e.key === "Escape" && overlay.classList.contains("rpk-open")) close();
      });
    }
    ...
    function open(opts) {
      ...
      payload = opts.text || "";
      const head = overlay.querySelector(".rpk-head");
      head.className = "rpk-head " + (mode === "call" ? "rpk-call" : "rpk-wa");
      ...
      const first = list.querySelector(".rpk-opt");
    ...
    function close() {
      if (!overlay) return;
      overlay.classList.remove("rpk-open");
